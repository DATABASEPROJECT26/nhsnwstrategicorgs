...
for sheet_name in wb.sheetnames:
    ...
    for i, row in enumerate(rows, 1):
        ...
        if i == 9 and row[2]:
            analysis['VISION'] = str(row[2]).strip()
        if i == 10 and row[2]:
            analysis['DELIVERY_SHORT'] = str(row[2]).strip()
        if i == 11 and row[2]:
            analysis['AMBITIONS'] = str(row[2]).strip()
        if i == 12 and row[2]:
            analysis['ENABLERS'] = str(row[2]).strip()

    dim_keys = {13: 'GEOGRAPHY', 14: '10YP_ALIGNMENT', 15: '10YP_DELIVERABILITY', 16: 'TRADE-OFFS',
                17: 'PREVENTION_READINESS', 18: 'DIGITAL_READINESS', 19: 'CARE_CLOSE_TO_HOME',
                20: 'WORKFORCE_READINESS', 21: 'EQUITY_READINESS', 22: 'INNOVATION_TRANSFORMATION', 23: 'ADAPTABILITY'}
    for rn in range(13, 24):
        if rn <= len(rows):
            row = rows[rn - 1]
            if row[2]:
                analysis[dim_keys[rn]] = str(row[2]).strip()

    other_items = []
    for rn in range(24, len(rows) + 1):
        if rn <= len(rows):
            row = rows[rn - 1]
            if row[1] and str(row[1]).strip().upper() == 'DELIVERY':
                break
            if row[2] and str(row[2]).strip():
                other_items.append(str(row[2]).strip())
    if other_items:
        analysis['OTHER'] = other_items
    ...
    if delivery_start:
        delivery = []
        for rn in range(delivery_start, len(rows) + 1):
            row = rows[rn - 1]
...
analysis_to_contact = {
    ...
    'Cheshire & Merseyside ICB': None,  # ICB, no contacts
    ...
    'University Hospitals of Liverpool Group (UHLG) currently comprised of Liverpool University Hospital (LUHFT), Liverpool Women\u2019s (LWH) and Liverpool Heart and Chest (LHCH)': None,  # Covers 3 orgs
...
for a_name, a_data in sorted(analysis_orgs.items()):
    ...
    if target is None:
        continue  # ICB or special entries - skip
...
# Special: University Hospitals of Liverpool Group - split analysis across 3 orgs
...
# 5. GENERATE HTML
# ===========================
# Custom org name overrides for display
org_display_override = {
    'North Cheshire and Mersey NHSFT - Warrington and Halton Teaching Hospitals NHSFT (WHH) and Bridgewater Community Healthcare NHSFT (BCH) will become a single organisation from April 2026': 'North Cheshire and Mersey NHSFT (Warrington & Halton / Bridgewater merger)'
}
...
print(f'Organisations in database: {len(org_map)}')
print(f'Total contacts: {len(all_contacts)}')
print()
print('Final listing:')
for k in sorted(org_map.keys()):
    o = org_map[k]
    has = ' [ANALYSIS]' if o.get('analysis') else ''
    print(f'  [{o["region"]:3s}] {k}{has} ({len(o["contacts"])} contacts)')
